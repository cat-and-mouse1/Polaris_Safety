...
public class ResultActivity extends BaseActivity {
    ...
    private void bindAiVerdict(View itemView, AppRiskInfo info) {
        ...
        if (v.reason != null && !v.reason.isEmpty()) {
            reasonText.append(getString(R.string.ai_reason_prefix, v.reason));
        }
        if (info.mlScore >= 0) {
            reasonText.append("\n[ML Detection] Score: ")
                    .append(info.mlScore).append("/100");
            if (info.mlReason != null && !info.mlReason.isEmpty()) {
                reasonText.append(" — ").append(info.mlReason);
            }
        }
    ...
    /** 显示单个应用的 ML 检测结果对话框。 */
    private void showMlResultDialog(AppRiskInfo info) {
    ...
    private class RiskAdapter extends RecyclerView.Adapter<RiskAdapter.VH> {
        ...
        public void onBindViewHolder(@NonNull VH holder, int position) {
            ...
            // ML 检测等级徽标（映射为风险等级文案，与风险等级 chip 同色）
            if (info.mlScore >= 0) {
                int mlLevel;
                if (info.mlScore >= 80) mlLevel = AppRiskInfo.LEVEL_HIGH;
                else if (info.mlScore >= 45) mlLevel = AppRiskInfo.LEVEL_MEDIUM;
                else if (info.mlScore >= 20) mlLevel = AppRiskInfo.LEVEL_LOW;
                else mlLevel = AppRiskInfo.LEVEL_SAFE;
                ...
                holder.mlBadge.setVisibility(View.VISIBLE);
                holder.mlBadge.setText("ML: " + mlLabel);
                holder.mlBadge.setTextColor(android.graphics.Color.WHITE);
                holder.mlBadge.setBackgroundTintList(ColorStateList.valueOf(mlColor));

                // ML 结果按钮 -> "查看"
                holder.mlBtn.setVisibility(View.VISIBLE);
                holder.mlBtn.setText("查看");
                holder.mlBtn.setOnClickListener(v -> showMlResultDialog(info));
            } else {
                holder.mlBadge.setVisibility(View.GONE);
                holder.mlBtn.setVisibility(View.GONE);
            }
        ...
        class VH extends RecyclerView.ViewHolder {
            ImageView icon;
            TextView name, pkg, score, reason, chip, mlBadge;
            ...
            VH(View v) {
                ...
                mlBadge = v.findViewById(R.id.mlBadge);
                handle = v.findViewById(R.id.handleButton);
                mlBtn = v.findViewById(R.id.mlResultBtn);
            }
        }
    }
}
