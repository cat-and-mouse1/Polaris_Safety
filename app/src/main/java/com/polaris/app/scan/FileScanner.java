...
public class FileScanner {
    ...
    /** point（内置精选库）：轻量（缓存或内置种子），构造时即加载。 */
    private final IocDatabase iocPoint;
    /**
     * region（全量库）：全量缓存可达 ~341MB，解析很重，
     * 延迟到扫描后台线程首次命中时加载（volatile + DCL），避免在主线程构造 FileScanner 时 ANR。
     */
    private volatile IocDatabase iocRegion;
    private final MlScanner mlScanner;
    private int scanned = 0;

    public FileScanner(Context context) {
        this.context = context.getApplicationContext();
        this.prefs = new Prefs(context);
        this.iocPoint = IocDatabase.getInstance(context, "point");
        this.mlScanner = new MlScanner(context);
        // iocRegion 懒加载，见 regionDb()：region 无论下载进度都参与检测
    }

    /** 懒加载 region 全量库；首次在扫描后台线程触发解析，避免阻塞主线程。 */
    private IocDatabase regionDb() {
        IocDatabase r = iocRegion;
        if (r == null) {
            synchronized (this) {
                r = iocRegion;
                if (r == null) {
                    r = IocDatabase.getInstance(context, "region");
                    iocRegion = r;
                }
            }
        }
    ...
    public void scanDirectory(File root, Callback cb) {
        ...
        new Thread(() -> {
            try {
                ...
                Log.e(TAG, "scan directory crashed", t);
            }
            finish(out, cb);
        }).start();
    }
    ...
    public void scanTree(Uri treeUri, Callback cb) {
        ...
        new Thread(() -> {
            try {
                ...
                Log.e(TAG, "scan tree crashed", t);
            }
            finish(out, cb);
        }).start();
    }

    /** SAF 多文件夹扫描：依次遍历多个授权树，全部完成后只回调一次。 */
    public void scanTrees(List<Uri> treeUris, Callback cb) {
        final List<FileRiskInfo> out = new ArrayList<>();
        new Thread(() -> {
            try {
                if (treeUris != null) {
                ...
                Log.e(TAG, "scan trees crashed", t);
            }
            finish(out, cb);
        }).start();
    }
    ...
    private void walkTree(Uri treeUri, Uri dirUri, List<FileRiskInfo> out,
        ...
        try {
            ...
            Log.w(TAG, "build children uri failed: " + dirUri, e);
            return;
        }
    ...
    private FileRiskInfo evaluateFile(String path, String name, long size, byte[] header) {
        ...
        for (String knownPkg : MalwareScanner.knownPackageNames(context)) {
            String pkgKey = knownPkg.replace('.', ' ');
            if (nameKey.contains(pkgKey)) {
                score += 45;
                info.reasons.add("文件名命中已知恶意家族：" + MalwareScanner.lookUpFamily(knownPkg, context));
                break;
            }
        }
        ...
        // 8) 开源情报融合：region（全量，已下载多少用多少）+ point 双库并行，
        //    region 无论下载进度都参与检测；SHA-256 优先，包名兜底，命中取风险更高者。
        if (isHashableExt(ext)) {
            String hash = computeSha256(path);
            if (hash != null) {
                info.sha256 = hash;
                String candPkg = extractPkgToken(lower);
                IocDatabase.IocEntry hit = bestIocHit(hash, candPkg);
                if (hit != null) {
        ...
        // ML scoring for APK files (additive signal, 30% weight)
        if (info.isApk && mlScanner != null && mlScanner.isInitialized()) {
            float[] features = extractFileFeatures(path);
            float probability = mlScanner.classify(features);
            int mlScore = (int) (probability * 100);
            info.mlScore = mlScore;
            
            // Generate reason
            if (probability >= 0.5f) {
                info.mlReason = String.format("ML detected malware (confidence: %.1f%%)", probability * 100);
            } else {
                info.mlReason = String.format("ML classified as benign (confidence: %.1f%%)", (1 - probability) * 100);
            }
            
            // Add ML score with 30% weight (unconditional)
            int mlWeighted = (int) (mlScore * 0.3);
            score += mlWeighted;
            info.reasons.add("[ML] " + info.mlReason);
        } else {
            info.mlScore = -1;
    ...
    /**
     * 在 region（全量库）与 point（内置精选库）中并行查询，返回风险更高的命中。
     * region 无论下载进度如何都参与：全量缓存下覆盖最新 IOC，未下载/下载中则回退内置种子；
     * 单库内仍保持「SHA-256 优先、包名兜底」。
     */
    private IocDatabase.IocEntry bestIocHit(String hash, String pkg) {
        IocDatabase.IocEntry best = null;
        int bestScore = -1;
        for (IocDatabase db : new IocDatabase[]{regionDb(), iocPoint}) {
            if (db == null) continue;
            IocDatabase.IocEntry h = db.matchByHash(hash);
            if (h == null && pkg != null) h = db.matchByPkg(pkg);
            if (h == null) continue;
            int s = IocDatabase.severityScore(h.severity);
            if (s > bestScore) {
                bestScore = s;
                best = h;
            }
    ...
    /**
     * Extract MH-100K features from a file path.
     * Simplified feature extraction for file-level analysis.
     */
    private float[] extractFileFeatures(String path) {
        float[] features = new float[24]; // MH-100K has 24 features
        
        try {
            File file = path.startsWith("content:") ? null : new File(path);
            if (file == null || !file.exists()) return features;
            
            // Feature 0: File size (normalized)
            features[0] = Math.min(file.length() / (1024f * 1024f), 200f) / 200f;
            
            // Feature 1: Is APK file
            features[1] = path.toLowerCase(Locale.US).endsWith(".apk") ? 1.0f : 0.0f;
            
            // Features 2-23: File-based heuristics (simplified)
            // In production, parse APK manifest for actual permissions/APIs
